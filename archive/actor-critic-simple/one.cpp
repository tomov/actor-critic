...
void trial(bool do_print)
{
    ...
    if (do_print) cout<<"\n  ---------------------- TRIAL --------------\n\n";
    int count = 0;
    double bias = 60; // TODO sketchy... #hardcoded
    double delta_prev = 0;
...
void print_4de(bool matlab = true)
{
     double juice_delta[256];
     memset(juice_delta, 0, sizeof(juice_delta));
     for (int i = 0; i < N; i++)
     {
         state_t &S = state[i];
         if (S.name.length() == 8 && S.name.substr(0, 6) == "reward")
         {
             char cue_id = S.name[7];
             transition_t &get_juice = S.next[0];
             juice_delta[cue_id] = get_juice.delta_avg;
         }
     }
 
     vector<pair<double, pair<pair<int, int>, double> > > Ds_a;
     vector<pair<double, pair<pair<int, int>, pair<double, double> > > > Ds_b;
     for (int i = 0; i < N; i++)
     {
         state_t &S = state[i];
         if (S.name.length() == 5 && S.name.substr(0, 2) == "go")
         {
             ...
             transition_t &low_trans = S.next[0];
             transition_t &high_trans = S.next[1];
             double Dlow = juice_delta[low_cue_id]; 
             double Dhigh = juice_delta[high_cue_id]; 

             double Davg = (Dlow * low_trans.times + Dhigh * high_trans.times) / (low_trans.times + high_trans.times);
             double expRlow = cue[low_cue_id].exp_reward_hardcoded;
             double expRhigh = cue[high_cue_id].exp_reward_hardcoded;
             double expR = (expRlow + expRhigh) / 2;  // TODO OMG WTF INVESTIGATE WHY DO THEY are averaging that way.... and not probability matching, which is how the monkey will see it
             Ds_a.push_back(make_pair(expR, make_pair(make_pair(expRhigh, expRlow), Davg)));
             if (low_cue_id != high_cue_id)
             {
                 Ds_b.push_back(make_pair(expR, make_pair(make_pair(expRhigh, expRlow), make_pair(Dhigh, Dlow))));
             }
         }
     }
     sort(Ds_a.begin(), Ds_a.end());
     ...
     if (matlab)
     {
         ...
         for (int i = 0; i < Ds_a.size(); i++)
         {
             cout<<"'"<<Ds_a[i].second.first.first<<"-"<<Ds_a[i].second.first.second<<"', ";
         }
         cout<<"}\n";
         cout<<"y4a = [";
         for (int i = 0; i < Ds_a.size(); i++)
         {
             cout<<Ds_a[i].second.second<<", ";
         }
         cout<<"]\n";
     }
     if (matlab)
...
void print_4f(bool matlab = true)
{
    cout<<"\n %% ---- figure 4f ---- \n\n";
    ...
    for (char cue_id = 'A'; cue_id <= 'D'; cue_id++)
    {
        double R = (double)cue[cue_id].rewarded / cue[cue_id].seen;
        xs_ref.push_back(R * 100); // TODO HARDCODED
        ys_ref.push_back(juice_delta[cue_id]);
    }
    ...
    for (char cue_id = 'A'; cue_id <= 'D'; cue_id++)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (S.name.length() == 5 && S.name.substr(0, 2) == "go")
            {
                ...
                {
                    count++;
                    delta_avg += juice_delta[low_cue_id];
                }
                ...
                {
                    count++;
                    delta_avg += juice_delta[high_cue_id]; 
                }
            }
        }
        delta_avg /= count;
